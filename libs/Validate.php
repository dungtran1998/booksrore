...
class Validate
{
	...
	public function showErrorsPublic()
	{
		...
		if (!empty($this->errors)) {
			...
			foreach ($this->errors as $key => $value) {
				$xhtml .= '<li class="text-white"><b>' .  $value . '</li>';
	...
	{
		$xhtml = '<div class="alert alert-danger alert-dismissible" style="background-color:red;">
					<button type="button" class="close" data-dismiss="alert" aria-hidden="true">×</button>
					<h5><i class="icon fas fa-exclamation-triangle"></i> Lỗi!</h5>';
		if (!empty($this->errors)) {
			$xhtml .= '<ul class="list-unstyled mb-0">';
	...
	private function validatePassword($element, $options)
	{
		if ($options['action'] == 'add' || ($options['action'] == 'edit' && $this->source[$element])) {
			$pattern = '#.*^(?=.{8,20})(?=.*[a-z])(?=.*[A-Z])(?=.*[0-9])(?=.*\W).*$#';	// Php4567!
			if (!preg_match($pattern, $this->source[$element])) {
				$this->setError($element, 'must be at least 8 characters in length and must contain at least one number, one upper case letter, one lower case letter and one special character.";
				');
			};
		}
	}
